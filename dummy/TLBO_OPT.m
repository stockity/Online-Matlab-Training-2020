function [bestFitness,bestSol] = TLBO_OPT(FitnessFunction,lowerBound,upperBound)
% clc;          % clear the command screen and ram data

%% Problem Settings
lb= lowerBound;          % set lower bound limit of variables   
ub=upperBound;         % Set Upper bound limit of variables
prob = @(x) FitnessFunction(x);     % call the fitness function
...
P=repmat(lb,Np,1)+repmat(1,Np,1).*randi([lb ub],Np,D); % Initialize Population with random number between lb and Ub
...
for t=1:T
    ...
    for i=1:Np
        %% Teacher Phase
        Xmean = floor(mean(P));    %Determine mean of the Population
        
        [~,ind]=min(f);     %Determine the location of Teacher
        Xbest = P(ind,:);   %Copying the location of teacher
        
        TF=randi([1,2],1,1);    % Generate either 1 or 2 randomly for Teaching Factor
        
        Xnew = floor(P(i,:) + rand(1,D).*(Xbest-TF*Xmean)); % Generate the New Solution Variable
        ...
        if f(i)<f(p)        %Select appropriate equation to be used in Learner Phase
            Xnew = floor(P(i,:) +rand(1,D).*(P(i,:)-P(p,:))); % Generate the new solution set
        else
            Xnew = floor(P(i,:)- rand(1,D).*(P(i,:)-P(p,:))); % Generate the new solution set
        end
...
Population_and_result =[P f]
[bestFitness,ind] = min(f);
bestSol = P(ind,:);

subplot(1,2,1);
plot(0:T,BestFitIter,'LineWidth',2);
xlabel('No. of Iterations -> ');ylabel('Best Fitness Function Value');
title('Optimazation sequence');
legend('Fitness score');

subplot(1,2,2);
semilogy(0:T,BestFitIter,'Color',[0.16 0.38 0.27],'LineWidth',2);
xlabel('No. of Iterations -> ');ylabel('Best Fitness Function Value');
title('Logrithmic Optimazation sequence ');
legend('Fitness score');
